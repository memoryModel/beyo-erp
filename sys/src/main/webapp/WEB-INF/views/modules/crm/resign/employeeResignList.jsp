...
<ul class="nav nav-tabs">
    <li class="active"><a href="${ctx}/crm/employeeResign/list">待入职列表</a></li>

</ul>
<form:form id="searchForm" modelAttribute="entry" action="${ctx}/crm/employeeResign/list" method="post" class="breadcrumb form-search">
    ...
    <c:forEach items="${page.list}" var="entry">
        <tr>
            ...
            <td>
                ...
                <c:if test="${entry.approveStatus == 3}"><%--未提交--%>
                    <shiro:hasPermission name="crm:employeeResign:getEmployeeInfo"><a href="${ctx}/crm/employeeResign/form?id=${entry.id}">办理入职</a></shiro:hasPermission>
                </c:if>
                <c:if test="${entry.approveStatus == 4}"><%--已退回--%>
                    <shiro:hasPermission name="crm:employeeResign:getEmployeeInfo"><a href="${ctx}/crm/employeeResign/form?id=${entry.id}">修改办理</a></shiro:hasPermission>
                </c:if>
<%--
                <shiro:hasPermission name="crm:employeeResign:delete"><a href="${ctx}/crm/employeeResign/delete?id=${employee.id}" onclick="return confirmx('确认要解约该员工吗？', this.href)">解约</a></shiro:hasPermission>
--%>
