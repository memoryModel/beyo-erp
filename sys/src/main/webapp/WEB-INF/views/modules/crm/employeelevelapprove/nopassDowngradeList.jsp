...
<ul class="nav nav-tabs">
	<li class="active"><a href="${ctx}/crm/employeeGradeApprove/nopassDowngradeList">升降级审批</a></li>
</ul>
<form:form id="searchForm" modelAttribute="employeeGrade" action="${ctx}/crm/employeeGradeApprove/nopassDowngradeList" method="post" class="breadcrumb form-search">
	...
	<ul class="ul-form">
		...
		<li>
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;降级技能项：
			<form:select path="skill.id"  class="input-medium">
				<form:option value="" label="------请选择------"/>
...
<table id="contentTable" class="table table-striped table-bordered table-condensed">
	...
	<c:forEach items="${page.list}" var="employeeGrade">
		<tr>
			...
			<td>
					${employeeGrade.downgradeLevelName}
			</td>
			<td>
					${erp:getGradeStatusName(employeeGrade.gradeStatus)}
			</td>
			<td>
					${employeeGrade.createUserName}
			</td>
			<td>
			...
			<td>
				<c:if test="${employeeGrade.gradeStatus == 2}">
					<shiro:hasPermission name="crm:employeeGradeApprove:view"><a href="${ctx}/crm/employeeGradeApprove/updateDowngradeForm?id=${employeeGrade.id}">修改测评</a></shiro:hasPermission>
				</c:if>
			</td>
		</tr>
	</c:forEach>
