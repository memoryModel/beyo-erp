...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			lod();radioCheck();subDeadlineTime();
		});
        ...
        function subDeadlineTime(){
            var date = new Date($('#signingDeadlineTme').val());
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/crm/employeeRenew/applicationCancellation?id=$${employeeRenew.id}">员工解约</a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="employeeCancellation" action="${ctx}/crm/employeeCancellation/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		<input type="hidden" name="employeeEntry.id" value="${employeeCancellation.employeeEntry.id}">
		<input type="hidden" name="tagFlag" value="1">
		...
		<div class="control-group">
			...
			<div class="controls">
				<input name="employeeEntry.deadlineTime" id="signingDeadlineTme" type="text" disabled="disabled" readonly="readonly" maxlength="20" class="input-medium Wdate "
		...
		<div class="control-group">
			...
			<div class="controls">
				<input name="createTime" type="text" readonly="readonly" disabled="disabled" maxlength="20" class="input-medium Wdate "
					   value="<fmt:formatDate value="${employeeCancellation.employeeEntry.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					   onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:true});"/>
			</div>
		</div>
		...
		<div class="control-group">
			...
			<div class="controls">
				<input name="cancellationEffectiveTime" id="cancellationEffectiveTimeId" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate "
					   value="<fmt:formatDate value="${employeeCancellation.cancellationEffectiveTime}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					   onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:true});"/>
			</div>
		</div>
		<div class="control-group">
