...
<head>
	...
	<script type="text/javascript">
        ...
        function selectEmployee(employeeId,name,phone,empCard){
            console.log("#btnSelect"+employeeId,name,phone,empCard);
            top.$.jBox.confirm('确认要选择添加该员工吗？','系统提示',function(v,h,f) {
                if (v == 'ok') {
                    parent.window.frames["mainFrame"].selectEmployeeCallback(employeeId,name,phone,empCard);
                    top.$.jBox.close(true);
                }
...
<body>
	<form:form id="searchForm" modelAttribute="employee" action="${ctx}/erp/packageDeposit/selectEmployee" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li><label>创建时间：</label>
				<input name="createTimeStart" type="text" maxlength="20" class="input-medium Wdate " readonly="readonly"
					   value="<fmt:formatDate value="${employee.createTimeStart}" pattern="yyyy-MM-dd"/>"
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="employee">
			<tr>
				...
				<td>
					<shiro:hasPermission name="erp:packageDeposit:selectEmployee"><a id="btnSelect" class="btn btn-primary" onclick="selectEmployee('${employee.id}','${employee.name}','${employee.phone}','${employee.idcard}')" href="javascript:; ">添加员工</a></shiro:hasPermission>
				</td>
			</tr>
		</c:forEach>
		</tbody>
