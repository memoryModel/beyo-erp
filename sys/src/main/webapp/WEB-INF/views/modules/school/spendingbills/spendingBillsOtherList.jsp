...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/erp/orderRefundOtherBill/">其他支出列表</a></li>
		<li class=""><a href="${ctx}/erp/orderRefundOtherBill/form">新增支出单</a></li>
	</ul>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tr>
			...
			<th>支出状态</th>
			<th>付款状态</th>
			<shiro:hasPermission name="orderrefund:orderRefund:edit"><th>操作</th></shiro:hasPermission>
		</tr>
		</thead>
		<tbody>
		<c:forEach items="${billOtherList}" var="orderRefund">
			<tr>
				<td>
				...
				<td>
					<fmt:formatDate value="${orderRefund.expenditureTime}" pattern="yyyy-MM-dd"/>
				</td>
				<td>
					${orderRefund.paymentEmployee.name}
				</td>
				<td>
					<fmt:formatDate value="${orderRefund.paymentTime}" pattern="yyyy-MM-dd"/>
				</td>
				<td>
				...
				<shiro:hasPermission name="orderrefund:orderRefund:edit"><td>
					<c:if test="${orderRefund.expenditureStatus == 1}">
						<a href="${ctx}/erp/orderRefundBill/spendingConfirm?id=${orderRefund.id}&&orderId=${orderRefund.order.id}">支出确认</a>
					</c:if>
					<c:if test="${orderRefund.expenditureStatus == 0&&orderRefund.paymentStatus==1}">
						<a href="${ctx}/erp/orderRefundBill/bill?orderId=${erpPaymentBill.order.id}">查看</a>
					</c:if>
