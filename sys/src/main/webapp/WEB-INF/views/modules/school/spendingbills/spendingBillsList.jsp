...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/erp/orderRefundBill/">支出账单列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="orderRefund" action="${ctx}/erp/orderRefundBill/" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li>
				<%--<label>业务归属人：</label>
						<sys:employeeselect id="employee" name="order.student.employee.id" value="${orderRefund.order.student.employee.id}" labelName="orderRefund.order.student.employee.name" labelValue="${orderRefund.order.student.employee.name}"
			...
			<li>
				...
				<input name="paymentTimeStart" type="text" maxlength="20" class="input-medium Wdate " readonly="readonly"
					   value="<fmt:formatDate value="${orderRefund.paymentTimeStart}" pattern="yyyy-MM-dd"/>"
					   onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:true});"/>-
				<input name="paymentTimeEnd" type="text" maxlength="20" class="input-medium Wdate " readonly="readonly"
					   value="<fmt:formatDate value="${orderRefund.paymentTimeEnd}" pattern="yyyy-MM-dd"/>"
					   onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:true});"/>
			</li>
			<li><label>支出状态：</label>
				<form:select path="expenditureStatus" class="input-large ">
					<form:option value="">请选择</form:option>
					<form:options items="${erp:expendStatusList()}" itemLabel="name" itemValue="value" htmlEscape="false"/>
				</form:select>
			</li>
			<li><label>付款状态：</label>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
		<c:forEach items="${billList}" var="orderRefund">
			<tr>
				<td>
				...
				<td>
					<c:if test="${orderRefund.expenditureStatus == 1}">
						<shiro:hasPermission name="erp:orderRefundBill:spendingConfirm"><a href="${ctx}/erp/orderRefundBill/spendingConfirm?id=${orderRefund.id}&&orderId=${orderRefund.order.id}">支出确认</a></shiro:hasPermission>
					</c:if>
					<c:if test="${orderRefund.expenditureStatus == 0&&orderRefund.paymentStatus==0}">
