...
<body>
	...
	<form:form id="searchForm" modelAttribute="employeeLeave" action="${ctx}/crm/employeeOvertime/list" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li><label>人员类别：</label>
				<form:select path="employee.type" class="input-medium">
	...
	<ul class="nav nav-tabs">
		<shiro:hasPermission name="crm:employeeLeave:form"><li>
			<a class="btn btn-primary" href="${ctx}/crm/employeeOvertime/form">加班添加</a>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="employeeLeave">
			<tr>
				...
				<td>
					${erp:employeeTypeName(employeeLeave.employee.type)}
				</td>
				<td>
				...
				<td>
					...
					<c:if test="${employeeLeave.status == 1}"><%--待客户确认--%>
						<shiro:hasPermission name="crm:employeeLeave:getEmployeeLeaveInfo"><a href="${ctx}/crm/employeeOvertime/getEmployeeLeaveInfo?id=${employeeLeave.id}">查看</a></shiro:hasPermission>
					</c:if>
					<c:if test="${employeeLeave.status == 2}"><%--客户已驳回--%>
						<shiro:hasPermission name="crm:employeeLeave:getEmployeeLeaveInfo"><a href="${ctx}/crm/employeeOvertime/getEmployeeLeaveInfo?id=${employeeLeave.id}">查看</a></shiro:hasPermission>
					</c:if>
					<c:if test="${employeeLeave.status == 3}"><%--待管理老师确认--%>
						<shiro:hasPermission name="crm:employeeLeave:getEmployeeLeaveInfo"><a href="${ctx}/crm/employeeOvertime/getEmployeeLeaveAgree?id=${employeeLeave.id}">审批</a></shiro:hasPermission>
					</c:if>
