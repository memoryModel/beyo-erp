...
<ul class="nav nav-tabs">
	<li class="active"><a href="${ctx}/crm/renewApproved/list">员工续约列表</a></li>
</ul>
<form:form id="searchForm" modelAttribute="renewApproved" action="${ctx}/crm/renewApproved/list" method="post" class="breadcrumb form-search">
	...
	<ul class="ul-form">
		...
		<li>续约截止日期：
			<input name="employeeRenew.extensionDeadlineStartDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
				   value="<fmt:formatDate value="${renewApproved.employeeRenew.extensionDeadlineStartDate}" pattern="yyyy-MM-dd HH:mm"/>"
			...
			<input name="employeeRenew.extensionDeadlineEndDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
				   value="<fmt:formatDate value="${renewApproved.employeeRenew.extensionDeadlineEndDate}" pattern="yyyy-MM-dd HH:mm"/>"
				   onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm',isShowClear:true});"/>
		</li>
...
<table id="contentTable" class="table table-striped table-bordered table-condensed">
	...
	<c:forEach items="${page.list}" var="renewApproved">
		<tr>
			...
			<td><fmt:formatDate value="${renewApproved.employeeRenew.createTime}" pattern="yyyy-MM-dd HH:mm"/></td>
			<td><a href="${ctx}/crm/renewApproved/form?id=${renewApproved.id}&myStatus=2">审批</a></td>
		</tr>
	</c:forEach>
	</tbody>
</table>
