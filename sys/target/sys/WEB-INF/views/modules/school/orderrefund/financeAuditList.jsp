...
<body>
<form:form id="searchForm" modelAttribute="order" action="${ctx}/erp/orderRefund/financeAuditLIst" method="post" class="breadcrumb form-search">
	...
	<ul class="ul-form">
		...
		<li>财务审核时间：
			<input name="financeAuditTimeStart" type="text" maxlength="20" class="input-medium Wdate " readonly="readonly"
				   value="<fmt:formatDate value="${orderRefund.financeAuditTimeStart}" pattern="yyyy-MM-dd"/>"
				   onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:true});"/>-
			<input name="financeAuditTimeEnd" type="text" maxlength="20" class="input-medium Wdate " readonly="readonly"
				   value="<fmt:formatDate value="${orderRefund.financeAuditTimeEnd}" pattern="yyyy-MM-dd"/>"
				   onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:true});"/>
		</li>
...
<table id="contentTable" class="table table-striped table-bordered table-condensed">
	...
	<c:forEach items="${page.list}" var="orderRefund">
		<tr>
			...
			<td><fmt:formatDate value="${orderRefund.financeAuditTime}" pattern="yyyy-MM-dd HH:mm"/></td>
			<td>
