...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/school/examineStudents/list">成绩查询</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="examineStudents" action="${ctx}/school/examineStudents/list" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li><label>是否及格：</label>
				<form:select path="passStatus" class="input-medium">
					<form:option value="" label="------请选择------"/>
					<form:options items="${erp:passStatusList()}" itemLabel="name" itemValue="value" htmlEscape="false"/>
				</form:select>
			</li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="examineStudents">
			<tr createUser="${examineStudents.createUser}" id="${examineStudents.id}">
				...
				<td>
					<fmt:formatDate value="${examineStudents.createTime}" pattern="yyyy-MM-dd HH:mm"/>
				</td>
				<td>
					<shiro:hasPermission name="school:examineStudents:form"><a href="${ctx}/school/examineStudents/form?id=${examineStudents.id}">修改成绩</a></shiro:hasPermission>
				</td>
			</tr>
		</c:forEach>
		</tbody>
