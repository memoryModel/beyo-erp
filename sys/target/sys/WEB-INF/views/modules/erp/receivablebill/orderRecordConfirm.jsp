...
<head>
	...
	<script type="text/javascript">
        $(document).ready(function() {
            /*alert(${classAmount.erpPaymentBill.id});*/
        });
        function page(n,s){
            if(n) $("#pageNo").val(n);
            if(s) $("#pageSize").val(s);
...
<div>
	<ul class="nav nav-tabs">
		...
		<li><a href="${ctx}/erp/paymentBillRecord/list">收款记账列表</a></li>
		<li class="active">
			<a href="${ctx}/erp/receivableBill/orderRecordInfo?billId=${classAmount.erpPaymentBill.id}&&orderId=${classAmount.id}">收款确认</a>
		</li>
	</ul></div><br/><div>

	<%--<form:form id="inputForm" modelAttribute="erpReceivableBill" action="${ctx}/erp/receivableBill/updateBillStatus?billId=${classAmount.erpPaymentBill.id}&&orderId=${classAmount.id}" method="post" class="form-horizontal">--%>
	...
	<div class="form-actions">
		...
		<shiro:hasPermission name="erp:receivableBill:updateBillStatus"><input id="btnSubmit" class="btn btn-primary" type="submit" value="收款确认"/>&nbsp;
		</shiro:hasPermission>
